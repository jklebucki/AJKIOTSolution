...
void readEEPROM(char *strToRead, int addrOffset)
{
  ...
  {
    yield(); // Umożliwia systemowi operacyjnemu obsługę innych zadań
...
void handleNewWiFiCredentials(const String &newSSID, const String &newPassword, const String &newDeviceId, const String &newMQTTServer)
{
  ...
  // Używamy pośredniego bufora i funkcji `strncat`
  char tempBuffer[MAX_TOPIC_LENGTH - 15];

  strncpy(tempBuffer, deviceId, sizeof(tempBuffer) - 1);
  tempBuffer[sizeof(tempBuffer) - 1] = '\0';
  snprintf(updateFeatureTopic, sizeof(updateFeatureTopic), "updateFeature/%s", tempBuffer);

  strncpy(tempBuffer, deviceId, sizeof(tempBuffer) - 1);
  tempBuffer[sizeof(tempBuffer) - 1] = '\0';
  snprintf(updateScheduleTopic, sizeof(updateScheduleTopic), "configSchedule/%s", tempBuffer);

  strncpy(tempBuffer, deviceId, sizeof(tempBuffer) - 1);
  tempBuffer[sizeof(tempBuffer) - 1] = '\0';
  snprintf(signalScheduleTopic, sizeof(signalScheduleTopic), "signalSchedule/%s", tempBuffer);

  strncpy(tempBuffer, deviceId, sizeof(tempBuffer) - 1);
  tempBuffer[sizeof(tempBuffer) - 1] = '\0';
  snprintf(configDeviceTopic, sizeof(configDeviceTopic), "configDevice/%s", tempBuffer);

  strncpy(tempBuffer, deviceId, sizeof(tempBuffer) - 1);
  tempBuffer[sizeof(tempBuffer) - 1] = '\0';
  snprintf(controlDeviceTopic, sizeof(controlDeviceTopic), "controlDevice/%s", tempBuffer);

  strncpy(tempBuffer, deviceId, sizeof(tempBuffer) - 1);
  tempBuffer[sizeof(tempBuffer) - 1] = '\0';
  snprintf(controlOnline, sizeof(controlOnline), "online:%s", tempBuffer);
...
void showEntries()
{
  ...
  {
    ScheduleEntry *entry = scheduleEntries.get(i);
    Serial.printf("Entry %d: Day: %d, StartTime: %s, EndTime: %s", i, entry->DayNumber, entry->timeToString(entry->StartTime).c_str(), entry->timeToString(entry->EndTime).c_str());
    Serial.printf(" - StartTime: %lld, EndTime: %lld", static_cast<long long>(entry->StartTime), static_cast<long long>(entry->EndTime));
    Serial.println();
    yield(); // Umożliwia systemowi operacyjnemu obsługę innych zadań
  }
}
...
void loadCertificates(WiFiClientSecure &client)
{
  ...
  if (loadCertFile("/client-cert.pem", [&client](Stream &stream, size_t size)
                   {
                     cert = new BearSSL::X509List(stream, size);
                     return true; }))
  {
  ...
  if (loadCertFile("/client-key.pem", [&client](Stream &stream, size_t size)
                   {
                     key = new BearSSL::PrivateKey(stream, size);
                     return true; }))
  {
  ...
  if (loadCertFile("/ca-cert.pem", [&client](Stream &stream, size_t size)
                   {
                     caCert = new BearSSL::X509List(stream, size);
                     return true; }))
  {
...
void debugSystemStatus()
{
  ...
  showEntries();
  Serial.printf("Feature: %s - Value: %d\n", feature.Name.c_str(), feature.Value);
  Serial.println();
}
